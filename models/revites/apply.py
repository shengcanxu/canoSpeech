...
def apply_model(model: tp.Union[BagOfModels, Model],
    ...
    kwargs: tp.Dict[str, tp.Any] = {
        ...
        'segment': segment,
        'lock': lock,
        'batch_size': batch_size  # for batch op
    }
    ...
    if shifts:
        ...
        with th.no_grad():
            # add batch logic
            split_wavs = torch.split(padded_mix, valid_length // batch_size, dim=-1)
            padded_mix = torch.concat(split_wavs, dim=0)
            out = model(padded_mix)
            split_wavs = torch.split(out, 1, dim=0)
            out = torch.cat(split_wavs, dim=-1)
