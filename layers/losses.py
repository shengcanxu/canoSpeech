...
class VitsGeneratorLoss(nn.Module):
    ...
    def forward(
        ...
        loss_kl = kl_loss(
            z_p=z_q_dur, logs_q=logs_q_audio, m_p=m_p_dur, logs_p=logs_p_dur, total_logdet=total_logdet, z_mask=z_mask.unsqueeze(1)
        ) * self.kl_loss_alpha
        ...
        if self.model_config.use_speaker_encoder_as_loss:
            loss_se = cosine_similarity_loss(gt_speaker_emb, syn_speaker_emb) * self.spk_encoder_loss_alpha
            loss = loss + loss_se
