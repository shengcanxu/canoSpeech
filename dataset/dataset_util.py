...
def stream_url(url: str, start_byte: Optional[int] = None, block_size: int = 32 * 1024, progress_bar: bool = True) -> Iterable:
    """Stream url by chunk
    ...
    with urllib.request.urlopen(req) as upointer, tqdm(unit="B", unit_scale=True, unit_divisor=1024, total=url_size, disable=not progress_bar, ) as pbar:
        num_bytes = 0
        while True:
...
def resample_file(func_args):
    ...
    if file_ext != "wav":
        filename += ".wav"
    audio.export(filename, format="wav", parameters=["-ar", "22050", "-ac", "1"])
