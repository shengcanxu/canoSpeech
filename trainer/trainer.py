...
class Trainer:
            ...
            if isinstance(optimizer, list):
                for idx, optim in enumerate(optimizer):
                    for group in optim.param_groups:
                        lr = self.get_lr(model, config)
                        group["lr"] = lr[idx] if isinstance(lr, list) else lr
            elif isinstance(optimizer, dict):
                for optim_name, optim in optimizer.items():
                    for group in optim.param_groups:
                        lr = self.get_lr(model, config)
                        group["lr"] = lr[optim_name] if isinstance(lr, dict) else lr
            else:
    ...
    def _compute_grad_norm(self, optimizer: torch.optim.Optimizer):
        ...
        # TODO: Don's know why param.grad is None, just filter it to make sure running works
